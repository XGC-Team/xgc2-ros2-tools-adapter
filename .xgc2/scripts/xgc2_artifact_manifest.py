#!/usr/bin/env python3
"""Create and verify ROS2 Tools trusted build manifests."""
...
def validate_identity(args: argparse.Namespace) -> None:
    if args.product != "xgc2-ros2-tools-adapter":
        raise ValueError("product identity is not xgc2-ros2-tools-adapter")
    if args.expected_package != "ros-jazzy-xgc2-ros2-tools-adapter":
        raise ValueError("expected package identity is invalid")
    if args.distribution != "noble":
        raise ValueError("distribution must be noble")
...
def verify_build(args: argparse.Namespace) -> None:
    ...
    if len(matches) != 1:
        raise ValueError(f"expected exactly one matching build manifest, found {len(matches)}")
